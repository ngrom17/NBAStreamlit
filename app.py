...
def _make_row(fav,dog,fs,ds,bk_data,best_h,best_h_bk,best_a,best_a_bk,sp,gt,extra,
    ...
    for bk_nm in ["draftkings","fanduel","betmgm"]:
        ...
        fml = bd["home_ml"] if fav_is_home else bd["away_ml"]
        if fml: bk_lines.append(f"{bk_nm[:2].upper()}: {fml:+.0f}")
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start">
    <div>
      ...
      <div class="dash-sub" style="margin-top:8px">
        ...
        <span class="source-pill">Odds · {len(odds_raw)} games</span>
        <span class="source-pill">Scores · ESPN</span>
        {'<span class="source-pill">🌤️ Weather</span>' if WEATHER_KEY else ''}
        {f'<span class="source-pill live">🔥 {streak_str}</span>' if streak_str else ''}
      </div>
      {last_night_html}
    </div>
    <div style="text-align:right;color:#5a6478;font-family:'DM Mono',monospace;font-size:0.8rem">
      {today_est().strftime("%b %d, %Y")}<br>{now_est()}
    </div>
  </div>
