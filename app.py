...
# ── KEYS / FILES ─────────────────────────────────────────────────────────────
ODDS_API_KEY = st.secrets.get("ODDS_API_KEY", "8762561865c3719f114b2d815aca3041")
...
def fetch_odds(sport_key):
    """Fetch odds from The Odds API. Returns [] on failure, but stores a readable error."""
    try:
        r = requests.get(
        ...
        try:
            ...
            st.session_state["odds_error"] = f"Odds API JSON error: {e}"
            return []
        if isinstance(d, dict):
            msg = d.get("message") or d.get("error") or str(d)[:200]
            st.session_state["odds_error"] = f"Odds API error ({r.status_code}): {msg}"
            return []
        st.session_state["odds_error"] = ""
        return d if isinstance(d, list) else []
    except Exception as e:
        st.session_state["odds_error"] = f"Odds API request failed: {e}"
        return []
...
# ── SESSION STATE ─────────────────────────────────────────────────────────────
st.session_state.setdefault("odds_open", {})
st.session_state.setdefault("odds_error","")
...
with st.spinner(f"Loading {sl}…"):
    ...
    if st.session_state.get("odds_error"):
        st.warning(f"Odds feed issue: {st.session_state['odds_error']}")
...
st.markdown(f"**{em} {sl}** · {today_est().strftime('%b %d')} · {src_icon} {src} · odds {len(odds_raw)} · {len(games)} filtered picks")
if st.session_state.get("odds_error"):
    st.warning("Odds feed issue: " + st.session_state["odds_error"][:200])
...
with tabs[0]:
    if not games:
        # If odds feed is down (common on free Odds API tiers), show a clear banner + ESPN fallback schedule
        if len(odds_raw) == 0 or st.session_state.get("odds_error"):
            st.warning("No odds data available right now (Odds API free tier / quota / rate limit). Showing ESPN schedule instead.")
            if st.session_state.get("odds_error"):
                st.caption(f"Odds API: {st.session_state['odds_error']}")
            if espn_today:
                st.markdown("#### 📅 Today's Games (ESPN)")
                for sg in espn_today:
                    a = sg["away"]["short"]; h = sg["home"]["short"]
                    st.caption(f"{sg['gametime']} · {a} @ {h} · {sg['detail']}")
            else:
                st.info("No ESPN schedule available either. Try Refresh in a minute.")
        else:
